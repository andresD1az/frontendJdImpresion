...
export default function MyOrders() {
  ...
  const downloadInvoice = (id) => {
    alert('Descarga de factura no implementada aún (placeholder).')
  }

  const requestReturn = (id) => {
    alert('Solicitud de devolución no implementada aún (placeholder).')
  }
              ...
              {list.map(o => (
                <tr key={o.id} className="border-t">
                  ...
                  <td className="py-3 px-4">
                    <span className={`text-xs px-2 py-1 rounded ${o.status==='paid'?'bg-green-100 text-green-700':o.status==='shipped'?'bg-blue-100 text-blue-700':'bg-gray-100 text-gray-700'}`}>{o.status||'pending'}</span>
                  </td>
                  <td className="py-3 px-4">
