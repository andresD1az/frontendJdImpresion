...
export default function Bodega() {
  ...
  const [form, setForm] = useState({ sku:'', type:'ingreso', quantity:'', reason:'' })
  const [priceEdit, setPriceEdit] = useState({}) // id -> sale_price_cop
  const [priceReason, setPriceReason] = useState({}) // id -> reason

  const load = async () => {
    setLoading(true)
    try {
      const res = await api(`/manager/inventory/stock?area=bodega${q?`&q=${encodeURIComponent(q)}`:''}`, { token })
      const rows = res.stock || []
      // Si no hay stock, intentamos reconstruir desde los movimientos
      if (rows.length === 0) {
        try {
          await api('/manager/inventory/rebuild-stock', { method:'POST', token })
          const res2 = await api(`/manager/inventory/stock?area=bodega${q?`&q=${encodeURIComponent(q)}`:''}`, { token })
          setStock(res2.stock || [])
        } catch {
          setStock([])
        }
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      await api('/manager/inventory/movement', { method:'POST', token, body: { sku, area:'bodega', type: form.type, quantity: qty, reason: form.reason } })
      setMsg('Movimiento registrado')
      setForm({ sku:'', type:'ingreso', quantity:'', reason:'' })
      load()
    } catch (e) {
      const code = e?.data?.error
      const detail = code==='stock_negative' ? 'Stock insuficiente en bodega'
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <h1 className="text-2xl font-semibold mb-4">Bodega</h1>
      ...
      <div className="bg-white rounded shadow p-4 mb-4">
        ...
        <form className="grid grid-cols-1 md:grid-cols-4 gap-3" onSubmit={submit}>
          ...
          <input className="border rounded px-3 py-2" placeholder="Motivo (opcional)" value={form.reason} onChange={e=>setForm({...form, reason:e.target.value})} />
          <div className="md:col-span-4 flex gap-2">
            <button disabled={!form.sku||!form.quantity} className="bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white px-3 py-2 rounded">Guardar</button>
      ...
      <div className="bg-white rounded shadow p-4">
        <div className="flex gap-2 mb-3">
          <input className="border rounded px-3 py-2 flex-1" placeholder="Buscar en bodega (SKU o nombre)" value={q} onChange={e=>setQ(e.target.value)} />
        ...
        {loading ? 'Cargando...' : (
          <div className="overflow-auto">
            <table className="min-w-full">
              ...
              <tbody>
                {stock.map((r,i)=> (
                  <tr key={i} className="border-t" onClick={()=>setForm(f=>({ ...f, sku: r.sku }))}>
